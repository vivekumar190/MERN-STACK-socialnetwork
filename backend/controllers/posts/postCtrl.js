...
const createPost= asyncHandler(async (req,res)=>{
    ...
    if(isProfane){
        ...
        throw new Error('post contains profane Words User Policy violated are blocked Contact administrator');
    }
    const localpath=`http://192.168.1.4:5000/uploads/${req.file.filename}`;
 
    try {
        const post=await Post.create({...req.body,image:localpath,user:_id,title:req.body.title});
        if(req.body.society){
...
try {
    if(hasCategory){
        const Posts=await Post.find({category:hasCategory}).populate('user').populate('comments').populate('society').sort('-createdAt');
       res.json(Posts);
    }
    if(hasSociety){
        const Posts=await Post.find({society:hasSociety}).populate('user').populate('comments').populate('society').sort('-createdAt');
       res.json(Posts);
    }
    else{
        const Posts=await Post.find({}).populate('user').populate('comments').populate('society').sort('-createdAt');
        res.json(Posts);
    }
} catch (error) {
    res.json(error);
...
const AddLike=asyncHandler(async (req, res) => {
    ...
    const alreadyDisliked=post?.dislikes?.find(userId=>userId?.toString()===loginUser.toString());
    //remove the user from dislikes if exists 
    if(alreadyDisliked){
        const post=await Post.findByIdAndUpdate(postId,{$pull:{dislikes:loginUser},isDisliked:false},{new:true});
        res.json(post);
    }
    // console.log(alreadyDisliked);
    //Remove user if he has liked the post
    if(isLiked){
        const post=await Post.findByIdAndUpdate(postId,{$pull:{likes:loginUser},isLiked:false},{new:true});
      res.send(post);
    }
    else{
        // add to likes
        const post =await Post.findByIdAndUpdate(postId,{$push:{likes:loginUser},isLiked:true},{new :true});
        await User.findByIdAndUpdate(post.user._id,{$inc:{points:10}});
       res.json(post);
    }
});
...
const addDislike=asyncHandler(async(req, res) => {
 ...
 const alreadyliked=post?.likes?.find(userId=>userId?.toString()===loginUser?.toString());
...
if(isDisliked){
   ...
   res.json(post);
    ...
    const post =await Post.findByIdAndUpdate(postId,{$push:{dislikes:loginUser},isDisLiked:true},{new:true});
     res.json(post);
}
});
module.exports = {createPost,fetchallPosts,fetchSinglePost,updatePost,deletePost,AddLike,addDislike};