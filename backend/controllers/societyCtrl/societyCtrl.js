...
const fetchUserSocieteis = asyncHandler(async (req, res) => {
  ...
  try {
    const societies = await Society.find({ _id: realsocietyarray })
      .populate("members")
      .populate("president")
      .populate("posts")
      .populate("vicepresident")
...
const fetchSingleSociety = asyncHandler(async (req, res) => {
  const { id } = req.params;
  try {
    const societies = await Society.findById(id)
      .populate("members")
      .populate("awaitedmembers")
      .populate("president")
      .populate("posts")
      .populate("vicepresident")
      .populate("treasurer")
...
const addMemberToSociety = asyncHandler(async (req, res) => {
  ...
  const societytest = await Society.findById(societyId);
  if (
    !req?.user?._id.toString() == societytest?.president?._id.toString() ||
    !req?.user?._id.toString() == societytest?.vicepresident?._id.toString() ||
    !req?.user?._id.toString() == societytest?.treasurer?._id.toString()
  ) {
  ...
  try {
    const society = await Society.findByIdAndUpdate(
      req?.body?.societyId,
      { $push: { members: memberId }, $pull: { awaitedmembers: memberId } },
      { new: true }
    );
    await User.findByIdAndUpdate(memberId, {
      $inc: { points: 50 },
      society: societyId,
      $push: { memberof: societyId },
    });
    res.json(society);
  } catch (error) {
    res.json(error);
...
const removeMemberfromSociety = asyncHandler(async (req, res) => {
  ...
  if (
    memberId.toString() === societytest?.president?._id.toString() ||
    memberId == societytest?.vicepresident?._id.toString()
  ) {
    throw new Error(
      "Vice president should be demoted first and then removed president cant be removed"
    );
  }
  if (
    !req?.user?._id.toString() == societytest?.president?._id.toString() ||
    ...
    throw new Error(
      "Only president or admin or tresurer or member can remove people"
    );
  }
  await Society.findByIdAndUpdate(req?.body?.societyId, {
    $push: { awaitedmembers: memberId },
  });
  try {
    const society = await Society.findByIdAndUpdate(
      req?.body?.societyId,
...
const addVicepresidentToSociety = asyncHandler(async (req, res) => {
  ...
  await Society.findByIdAndUpdate(req?.body?.societyId, {
    $push: { members: societytest?.vicepresident },
  });
  await User.findByIdAndUpdate(societytest?.vicepresident?._id, {
    $inc: { points: -100 },
  });
  try {
    const society = await Society.findByIdAndUpdate(
      req?.body?.societyId,
      { vicepresident: memberId, $pull: { members: memberId } },
      { new: true }
    );
    await User.findByIdAndUpdate(memberId, {
      $inc: { points: 100 },
...
const NewpresidentToSociety = asyncHandler(async (req, res) => {
  ...
  try {
    const society = await Society.findByIdAndUpdate(
      req?.body?.societyId,
      { president: memberId, vicepresident: _id },
      { new: true }
    );
    await User.findByIdAndUpdate(memberId, {
      $inc: { points: 200 },
...
const NewTresurerToSociety = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new Error(
      "Only president or vice-president can make some one Tresurer"
    );
  }
  if (!societytest?.members?.includes(memberId.toString())) {
    throw new Error("add him a member first a member");
  ...
  await Society.findByIdAndUpdate(req?.body?.societyId, {
    $push: { members: societytest?.treasurer },
  });
  await User.findByIdAndUpdate(societytest?.treasurer?._id, {
    $inc: { points: -100 },
  });
  try {
    const society = await Society.findByIdAndUpdate(
      req?.body?.societyId,
      { treasurer: memberId, $pull: { members: memberId } },
      { new: true }
    );
    await User.findByIdAndUpdate(memberId, {
      $inc: { points: 100 },
...
module.exports = {
  ...
  addVicepresidentToSociety,
  NewpresidentToSociety,
  NewTresurerToSociety,
  fetchSingleSociety,
};
