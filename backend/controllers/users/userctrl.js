...
const userProfile=asyncHandler(async (req,res)=>{
 ...
 validateMOongodbId(id);
  ...
  try {
  const myprofile=await User.findById(id).populate('posts').populate('memberof');
 ...
 const {_id}=req?.user;
  ...
  validateMOongodbId(_id);
    ...
    const user = await User.findByIdAndUpdate( _id,{
      ...
      lastName:req?.body?.lastName,
      email:req?.body?.email,
      bio:req?.body?.bio,},{new:true,runValidators:true});  // returned new updated user 
  res.json(user);
});
...
const followingUser= asyncHandler(async (req,res)=>{
 ...
 await User.findByIdAndUpdate(followId,{$push:{followers:loginUserId},isFollowing:true},{new:true});

 //2.update the login user following feild
 await User.findByIdAndUpdate(loginUserId,{$push:{following:followId}},{new:true});
...
const unfollowUser=asyncHandler(async (req, res)=>{ 
  ...
  await User.findByIdAndUpdate(unfollowId,{$pull:{followers:loginUserId},isFollowing:false},{new :true
  });

await User.findByIdAndUpdate(loginUserId,{$pull:{following:unfollowId}},{new :true});
...
const userFeed=asyncHandler(async(req,res)=>{
  ...
  try {
     const fromfollwings=await Post.find({ user: realarray }).populate('user').populate('comments').populate('society').sort('-createdAt');
     const fromsociety=await Post.find({ society: realsocietyarray }).populate('user').populate('comments').populate('society').sort('-createdAt');
...
const foundUser=await User.findByIdAndUpdate(_id,{profilePhoto:localpath},{new:true});
...
module.exports ={register,login,fetchallUsers,deleteUser,fetchUser,userProfile,UpdateUser,upateUserPassword,followingUser,unfollowUser,blockUser,unblockUser,generateverificationToken,accountVerification,forgetPassword,passwordUpdate,profilePhotoUpload,userFeed};